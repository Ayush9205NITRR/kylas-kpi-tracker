...
export async function run({ env = {}, log = () => {}, dry = false, date = "" } = {}) {
  ...
  const DATE = date || new Date(Date.now() - 864e5).toISOString().slice(0, 10);
  const today = new Date().toISOString().slice(0, 10);
  if (DATE >= today) {
    log(`${DATE} is not over yet. A day is counted once, after it ends.`);
    return { stopped: true };
  }
  ...
  for (const [k, v] of Object.entries(fields)) if (k !== "Key" && k !== "Frozen At") log(`  ${k.padEnd(28)} ${v}`);
...
if (isMain) {
  ...
  try {
    await run({ env: process.env, log: (...x) => console.log(...x),
                dry: argv.includes("--dry-run"), date: a("date") || "" });
