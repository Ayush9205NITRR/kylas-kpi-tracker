#!/usr/bin/env node
/* Generates the field validator for both runtimes.
 *
 *   node scripts/gen-fields.mjs
 *
 * WHY THIS EXISTS. Kylas answers a bad field with a 400 that names neither the
 * field nor the rule — `{"code":"002008","message":"Invalid Mobile Number."}`
 * for a phone number it will not spell out. An associate 140 calls into a day
 * cannot act on that, and the work they just typed is stuck behind it. So the
 * rules live here, in front of the write, and a rejected value is named and
 * fixable before anything leaves the machine.
 *
 * The console runs in a browser and the writers run in node. The rules exist
 * once, as the BODY below, and are emitted into both. Do not edit the
 * generated files: extension/console/fields.js, scripts/fields.mjs.
 */
import { writeFileSync } from "node:fs";

/* Everything inside BODY is plain JS, valid in both runtimes. No imports, no
...
const NAMES = ["DIAL_CODES", "ISO_OF", "FIELDS", "splitPhone", "e164", "prettyPhone",
               "checkPhone", "checkEmail", "checkName", "checkUrl", "splitName",
               "checkField", "checkContact", "isDate", "isTime"];

const HEAD = (how) => `/* GENERATED FROM scripts/gen-fields.mjs — do not edit.
   Regenerate with: node scripts/gen-fields.mjs
   ${how} */
`;

/* ── browser ─────────────────────────────────────────────────────── */
writeFileSync(new URL("../extension/console/fields.js", import.meta.url),
  HEAD("Loaded before console.js; the names below hang off window.Fields.") +
  "(function (global) {\n" + BODY + "\n" +
  `  global.Fields = { ${NAMES.join(", ")} };\n})(window);\n`);

/* ── node ────────────────────────────────────────────────────────── */
writeFileSync(new URL("./fields.mjs", import.meta.url),
  HEAD("Imported by kylas.mjs, proxy.mjs and the writers.") +
  BODY + "\n" + `export { ${NAMES.join(", ")} };\n`);

console.log(`${Object.keys(NAMES).length ? NAMES.length : 0} exports → extension/console/fields.js and scripts/fields.mjs`);
