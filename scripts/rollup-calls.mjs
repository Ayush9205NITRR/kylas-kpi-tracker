...
const DAYS = Number(arg("days") || process.env.CALL_RETAIN_DAYS || 90);
...
const cutoff = arg("before")
  ? day(arg("before"))
  : new Date(Date.now() - DAYS * 864e5).toISOString().slice(0, 10);

if (cutoff > today) { console.error(`Cutoff ${cutoff} is in the future.`); process.exit(1); }

console.log(APPLY ? "Call Log rollup" : "Call Log rollup — DRY RUN, nothing will be written or deleted");
console.log(`base ${BASE} · keeping raw calls from ${cutoff} onward` +
            (arg("before") ? "" : ` (${DAYS} days)`));
...
console.log(`  ${recent} within the window, left alone`);
if (undated) console.log(`  ! ${undated} row(s) carry no Called At — left alone, they cannot be dated`);
...
if (!days.length) console.log("\nNothing old enough to roll up.");
  ...
  console.log(`  ${totalOld} row(s) across ${days.length} day(s) older than ${cutoff}`);
}
...
for (const d of days) {
  ...
  if (!APPLY) {
    console.log(`  ${d}: ${byDay.get(d).length} row(s) -> ${out.length} aggregate(s)`);
    continue;
  }
  ...
  console.log(`  ${d}: ${byDay.get(d).length} row(s) -> ${out.length} aggregate(s), raw deleted`);
}
...
if (days.length) {
  ...
  else console.log(`Call Log is now ${rows.length - deleted} row(s).`);
}
...
if (keptDays >= 2) {
  ...
  for (const [plan, cap] of [["Team", 50_000], ["Business", 125_000]]) {
    if (steady > cap * 0.8)
      console.log(`  ! ${steady.toLocaleString()} is ${steady > cap ? "OVER" : "within 20% of"} ` +
